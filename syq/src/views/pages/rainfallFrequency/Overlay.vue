            ...
            <el-col :span="16">
                ...
                <el-row style="margin-bottom: 0">
                    ...
                    >
                        <span style="margin-right: 6px;">
                            累计雨量：
                            <i style="color: #33b2ff;"
                                >{{ options.data.max.total || 0 }}mm</i
                            >
                        </span>
                        <span
                            v-show="options.data.max.max1"
                            style="margin-right: 6px;"
                        >
                            最大1天：
                            <i style="color: #33b2ff;"
                                >{{ options.data.max.max1 }}mm</i
                            >
                        </span>
                        <span
                            v-show="options.data.max.max3"
                            style="margin-right: 6px;"
                        >
                            最大3天：
                            <i style="color: #33b2ff;"
                                >{{ options.data.max.max3 }}mm</i
                            >
                        </span>
                        <span
                            v-show="options.data.max.max5"
                            style="margin-right: 6px;"
                        >
                            最大5天：
                            <i style="color: #33b2ff;"
                                >{{ options.data.max.max5 }}mm</i
                            >
                        </span>
                        <span
                            v-show="!show && options.data.max.max7"
                            style="margin-right: 6px;"
                        >
                            最大7天：
                            <i style="color: #33b2ff;"
                                >{{ options.data.max.max7 }}mm</i
                            >
                        </span>
                    </el-col>
                </el-row>
                <el-row justify="center">
                    <el-col :span="24" align="center">
                        ...
                        <span v-show="show">
                            最大15天：
                            <i style="color: #33b2ff; margin-right: 6px;"
        ...
        >
            ...
            <el-table :data="overlay.history.tableData" border max-height="320">
                ...
                <el-table-column label="最大1天">
                    <el-table-column prop="mn" label="面雨量" />
...
<script lang="ts">
    ...
    export default class Overlay extends Vue {
        ...
        getChartData(): void {
            ...
            this.$http
                .get("/rest/rainFrequency/getDmHistoryData", { params })
                .then(({ data }) => {
                    ...
                    data.dataList.forEach((item: any) => {
                        ...
                        this.options.data.max.total += Number(
                            item.rain ? Number(item.rain).toFixed(1) : item.rain
                        );
        ...
        getFrequencyData(): void {
            this.$http
                .get(
                    "/rest/rainFrequency/getFrequencyResults?rcd=" +
                        this.prop.rcd
                )
                .then(({ data }) => {
                    if (data.length) {
        ...
        getHistoryData(): void {
            this.$http(
                ...
                this.overlay.history.tableData = Object.values(data).some(
        ...
        handleExport() {
            const params = {
                ...
                st: moment(this.options.date.start).format(
                    "YYYY-MM-DDTHH:mm:ss"
                ),
