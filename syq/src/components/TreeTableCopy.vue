    ...
    <div class="table-con">
        ...
        <div class="tb-main-fixed" v-if="!loading && xScroll">
            ...
            <virtual-list
                ...
                id="tb-content-fixed"
                @scroll.native="scrollHandler"
                :style="{ height: heightMixin - 11 + 'px' }"
            >
                <div
                    v-for="(child, ins) in visibleList"
                    :key="ins"
...
<script lang="ts">
    ...
    export default class TreeTable extends AutoHeightMixin {
                ...
                this.tableData.forEach(function(item: any) {
                    ...
                    item.children.forEach(function(sing: any) {
                        ...
                        if (sing.info.zl == "RR") {
                            ...
                            if (
                                sing.jj != "-" &&
                                Number(sing.sw) >= Number(sing.jj)
                            ) {
                                classes = "yellow";
                            }
                            if (
                                sing.bz != "-" &&
                                Number(sing.sw) >= Number(sing.bz)
                            ) {
                                classes = "red";
                            }
                        }
        ...
        get visibleList() {
            ...
            /* 编写一个函数，函数接受一个数值，若干数值区间。判断该数值是否在任一区间内
            1.handle(8,[7,10])// true
            2.handle(10,[1,5],[8,9])//false*/

            function isInArray(i: number, ...arr: number[]) {
                return arr.some((item: any) => item[1] > i && item[0] < i);
            }
        }
